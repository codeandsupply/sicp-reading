...
;; list that has a dot before the last parameter name indicates that,
;; when the procedure is called, the initial parameters (if any) will
...
(define (same-parity x . xs)
  (define (same-parity-list xs)
    (cond ((null? xs) nil)
          ((= (remainder x 2) (remainder (car xs) 2))
           (cons (car xs) (same-parity-list (cdr xs))))
          (else (same-parity-list (cdr xs)))))
  (cons x (same-parity-list xs)))
