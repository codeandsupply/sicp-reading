...
(define ll (make-branch 3 1))
(define lr (make-branch 2 6))
(define rl (make-branch 1 4))
(define rr (make-branch 1 2))
(define l (make-branch 1 (make-mobile ll lr)))
(define r (make-branch 3 (make-mobile rl rr)))
(define my-mobile (make-mobile l r))
...
(define (balanced? mobile)
  (cond ((not (pair? mobile)) #t)
        ((not (= (torque (left-branch mobile))
                 (torque (right-branch mobile)))) #f)
...
(define my-balanced-mobile
  (make-mobile (make-branch 3 (make-mobile (make-branch 4 2)
                                           (make-branch 2 4)))
               (make-branch 6 (make-mobile (make-branch 1 2)
                                           (make-branch 2 1)))))

(balanced? my-balanced-mobile)
...
;; You simply need to change your definitions for the selectors
;; left-branch, right-branch, branch-length, and branch-structure
;;
;; Specifically, you would need to remove the extra `car` step from
;; right-branch and branch-length
